...
import { getAllGenderProducts, getAllProducts } from "@/api/shop";
import ProductItem from "../index/productItem";

const GenderPage = ({slug}) => {
    ...
    const [paramsFilter , setParamsFilter] = useState("&color=&size=&category=&available=");
    ...
    useEffect(() => {
      if(slug){
        setParamsFilter("&color=" + filter?.color + "&size=" + filter?.size + "&category=" + filter?.category + "&available=" + filter?.available + "&subcategory=" + filter?.subcategory)
      }
    } , [filter , slug])
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getAllGenderProducts(currentPage,15,paramsFilter , slug);
    ...
    fetchData();
  }, [paramsFilter , slug , reload ,currentPage]);
    ...
    return(
        <div className={styles.categoryPage + " container max-w-7xl mx-auto my-20"}>
